...
export default function ServiceAreaChecker() {
  ...
  return (
    <div className="zip-checker">
      <p className="eyebrow">Check before booking</p>
      ...
      {result === "standard" && <div className="zip-result available" role="status"><strong>This ZIP is in our standard Charleston-area review zone.</strong><p>Continue to the cleaning planner. Your exact address, requested service, and provider availability are confirmed before the appointment.</p><Link className="button" href="/#pricing-tool">Build my clean →</Link></div>}
      {result === "review" && <div className="zip-result review" role="status"><strong>This ZIP needs a quick coverage review.</strong><p>We may still be able to help, especially with a recurring home or vacation-rental property. Send the address and service details to our Charleston team before booking.</p><Link className="button" href={`/contact?zip=${checkedZip}`}>Request coverage review →</Link></div>}
    </div>
  );
}
